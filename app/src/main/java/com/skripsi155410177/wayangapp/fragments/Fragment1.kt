...
class Fragment1 : Fragment() {
    ...
    fun read_json(){
        ...
        try{
            val inputStream: InputStream = getContext()!!.assets.open("data_dewa.json")
            ...
            for(i in 0 until jsonarr.length()-1){
                ...
                arr2.add(jsonobj.getString("kahyangan"))
            }

            var adp = ArrayAdapter(activity,android.R.layout.simple_list_item_1,arr)
            list1.adapter = adp

            list1.onItemClickListener = AdapterView.OnItemClickListener{ parent, view, position, id ->
                val toast = Toast.makeText(activity, arr[position]+" dari "+arr2[position], Toast.LENGTH_LONG)
